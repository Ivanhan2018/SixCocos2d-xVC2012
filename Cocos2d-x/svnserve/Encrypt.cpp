...
//生成密文
unsigned short CXOR6601Encrypt::EncryptData(const char* pszSrcData, char* pszEncrypData, unsigned short wSize,int encrypt_key_len)
{
	//效验参数
	assert(wSize>0);
	...
	szRandKey[0]=strlen(pszSrcData);
	for (unsigned short i=1;i<CountArray(szRandKey);i++) szRandKey[i]=rand()%255;
	...
	for (unsigned short i=0;i<wTimes;i++)
	{
		...
		snprintf(pszEncrypData+i*4,5,("%02x%02x"),szRandKey[i%encrypt_key_len],bTempCode);
	}
...
//解开密文
unsigned short CXOR6601Encrypt::CrevasseData(const char* pszEncrypData, char* pszSrcData, unsigned short wSize,int encrypt_key_len)
{
	//效验参数
	assert(wSize>0);
	assert(pszSrcData!=NULL);
	...
	for (int i=0;i<wSoureLength;i++)
	{
		...
		pszSrcData[i]=(char)((unsigned char)uKey^(unsigned char)uEncrypt);
	}
