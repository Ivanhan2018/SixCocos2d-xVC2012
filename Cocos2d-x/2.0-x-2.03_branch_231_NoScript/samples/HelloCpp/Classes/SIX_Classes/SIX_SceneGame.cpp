...
void SIX_SceneGame::onEnterTransitionDidFinish()
{
	...
	SIX_Utility::addSpriteFramesWithPlistAsync(CCString::create("Scene/Game/GunBulletGold.plist"), this,callfuncO_selector(SIX_SceneGame::AddImageAsyncCallBack));
	//金币转盘
...
void SIX_SceneGame::AddImageAsyncCallBack(CCObject *pObject)
{
	...
	if (pTexture)
	{
		...
		SIXLog("plist[%s]",plist->getCString());
		...
		SIXLog("[0x%08X]SIX_SceneGame.AddImageAsyncCallBack.l_LoadImageCount[%d].l_LoadImageCountTotal[%d]",
			SIX_Utility::GetInstance()->GetCurrentThreadId(),
			l_LoadImageCount,
			l_LoadImageCountTotal);
