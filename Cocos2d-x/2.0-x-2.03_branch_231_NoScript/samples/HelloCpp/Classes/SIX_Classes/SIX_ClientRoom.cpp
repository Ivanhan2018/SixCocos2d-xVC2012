...
void SIX_ClientRoom::Connect(const char *addr,int port,bool bReconnected,int iReconnectDelay)
{
	m_gClientRoom = SIX_ClientRoom::GetInstance();
#ifdef LOCAL_SERVER
	m_gClientRoom->init("127.0.0.1",port,SIX_PlayerMgr::GetInstance()->getMyName(),SIX_PlayerMgr::GetInstance()->getMyPass(),bReconnected,iReconnectDelay);
#else
...
void SIX_ClientRoom::HandlePacketRoom(int subCode,unsigned char *pData,int pDataLen)
{
...
#ifdef WIN32
				CCString *title = CCString::createWithFormat("%s - [%d.%d.%d.%04d] - %s[%s][%lld]",PROJECT_NAME,VER_MAIN,VER_CHILD,VER_STAGE,VER_BUILD,
					pUserInfo->szName,pUserInfo->szNickname,pUserInfo->lUserId);
	...
	case ROOM_SUB_TABLE_LIST:
		{
			...
			PlayerTreasureInfo::Instance().m_lGameType=packet->iRoomType;
	...
	case ROOM_SUB_USER_LIST_OVER:
		{
			...
			if (SIX_Utility::GetInstance()->IsSceneRunning(pSceneGate,"SIX_SceneGate"))
			{
				...
				pSceneGate->getRoomList()->ShowJoinRoom(false);
	...
	case ROOM_SUB_LEAVE_ROOM:
		{
			LEAVE_ROOM_RESULT *pUserInfo = (LEAVE_ROOM_RESULT*)pData;
			SIXLog("[%lld][%d/%d]离房成功",
				pUserInfo->lUserId,
				pUserInfo->iTableNo,
				pUserInfo->iSeatNo);
