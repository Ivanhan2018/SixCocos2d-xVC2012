...
void SIX_RoomList::ShowJoinRoom(bool bShow)
{
	if (!pLabelJoinRoom)
	{
		pLabelJoinRoom = CCLabelTTF::create();
		if (!pLabelJoinRoom)
			return;
		pLabelJoinRoom->ignoreAnchorPointForPosition(true);
		pLabelJoinRoom->setPosition(ccp(350,300));
		pLabelJoinRoom->setString(SIX_Utility::GetInstance()->G2U("正在进入房间，请稍候...").c_str());
		pLabelJoinRoom->setFontName("微软雅黑");
		pLabelJoinRoom->setFontSize(24);
		pLabelJoinRoom->setColor(ccc3(255,255,255));
		pLabelJoinRoom->setOpacity(255);
		SIX_Utility::GetInstance()->FixedTexParameters(pLabelJoinRoom->getTexture());
		this->addChild(pLabelJoinRoom);
	}
	pLabelJoinRoom->setVisible(bShow);
}
