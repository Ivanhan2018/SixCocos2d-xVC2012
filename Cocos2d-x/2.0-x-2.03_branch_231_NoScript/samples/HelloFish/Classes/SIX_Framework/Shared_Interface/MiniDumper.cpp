...
void CMiniDumper::SetUploadURL(char *PostURL)
{
	UploadUrl.clear();
	UploadUrl.append(PostURL);
}
...
void CMiniDumper::setMiniDumpFileName( void )
{
	...
	TCHAR fullDir[1024] = {0};
	wsprintf(fullDir,L"%s//Dump//",m_szAppPath);
	CreateDirectory(fullDir,0);

    wsprintf( m_szMiniDumpPath,
                 L"%s//Dump//%s_%04d%02d%02d_%02d%02d%02d_%04d.dmp",
                 m_szAppPath,
                 m_szAppBaseName,
				 st.wYear,
				 st.wMonth,
...
LONG CMiniDumper::writeMiniDump( _EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	HMODULE hDll = NULL;
	TCHAR szDbgHelpPath[1024];

	if( GetModuleFileName( NULL, m_szAppPath, 1023 ) )
	{
	...
	if( hDll )
	{
		...
		if( MiniDumpWriteDump != NULL )
        {
			...
			if( hFile != INVALID_HANDLE_VALUE )
			{
...
#ifdef CALL_BUGREPORT
				// ----------------------------
				// Call BugReport
				// ----------------------------
				// Add By Cool.Cat
				char szDir[1024] = {0};
				GetCurrentDirectory(1023, szDir);
				...
				string fullpath("");
				//fullpath.append("\"");
				fullpath.append(szDir);
				fullpath.append("\\");
				fullpath.append(/*iBugReport.exe*/XorStr<0xD6,15,0x3E05A02B>("\xBF\x95\xAD\xBE\x88\xBE\xAC\xB2\xAC\xAB\xCE\x84\x9A\x86"+0x3E05A02B).s);
				//fullpath.append("\"");

				string cmdArv("");
				if (UploadUrl.length()==0)
				{
					cmdArv.append(/*>*/XorStr<0x7A,2,0x395CC34C>("\x44"+0x395CC34C).s);
					//cmdArv.append("\"");
					cmdArv.append(m_szMiniDumpPath);
					//cmdArv.append("\"");
				}
				else
				{
					cmdArv.append(UploadUrl.c_str());
					cmdArv.append(/*>*/XorStr<0x7A,2,0x395CC34C>("\x44"+0x395CC34C).s);
					//cmdArv.append("\"");
					cmdArv.append(m_szMiniDumpPath);
					//cmdArv.append("\"");
				...
				//char info[1024] = {0};
				//sprintf_s(info,1023,/*fullpath = %s\ncmdArv = %s\ndirpath = %s*/XorStr<0x36,39,0x716F5531>("\x50\x42\x54\x55\x4A\x5A\x48\x55\x1E\x02\x60\x64\x31\x49\x27\x28\x22\x06\x3A\x3F\x6A\x76\x6C\x68\x3D\x45\x34\x38\x20\x23\x35\x21\x3E\x77\x65\x79\x7F\x28"+0x716F5531).s,
				//	fullpath.c_str(),
				//	cmdArv.c_str(),
				//	dirpath.c_str());
				//MessageBox(0,info,0,0);
				ShellExecute(0, 0, fullpath.c_str(), cmdArv.c_str(), dirpath.c_str(), SW_SHOWNORMAL);
#endif
			}
			else
			{
