...
㈢ 多边形常用算法模块   
1. 判断多边形是否简单多边形----issimple（FPOINT polygon[]理解为一个确定的多边形对象或有序的顶点集，结合Graham_scan函数使用）  
2. 检查多边形顶点的凸凹性 9   
3. 判断多边形是否凸多边形----isconvex（FPOINT polygon[]理解为一个确定的多边形对象或有序的顶点集，结合Graham_scan函数使用），isconvexEx（FPOINT polygon[]理解为一个确定的简单多边形对象或无序的顶点集）   
4. 求多边形面积 9   
...
(八)捕鱼客户端要用到的多边形碰撞算法模块，add by Ivan_han 20130603
1.判断圆是否与多边形相交----CircleCrossPolygon
2.判断直线是否与简单多边形相交----LineCrossPolygon
*/ 
#pragma once
//#include<cmath>
//#include<xutility>
...
/* 常用的常量定义 */    
const float    INF     = 1E20f;//无穷大       
const float    EP      = 1E-10f;//无穷小    
const int       MAXV    = 300;    
...
float dist(FPOINT p1,FPOINT p2)                // 返回两点之间欧氏距离     
{    
    return( sqrt( (p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y) ) );    
}    
bool equal_point(FPOINT p1,FPOINT p2)           // 判断两个点是否重合      
{    
...
bool lineintersect(LINE l1,LINE l2,FPOINT &p) // 是 L1，L2     
{    
...
本程序默认第一个条件已经满足
//issimple：
//对于能构成凸边形的四个点，pt[0],pt[1],pt[2],pt[3]构成一个简单多边形就返回true，否则就返回false，但是不能判断顶点顺序是逆时针，还是顺时针
//对于不能构成凸边形的四个点，pt[0],pt[1],pt[2],pt[3]构成一个简单多边形就返回true，否则就返回false，但是不能判断顶点顺序是逆时针，还是顺时针
*/    
...
int tt(float p[][2],int a,int b,int N) //a为下标，不能为float 
{  
	//int temp[N],k=0;
	int temp[MAXV],k=0;
	float t;  
...
//输入N个点的坐标,判断能否构成一个凸多边形
//与isconvex不同的是，isconvexEx不依赖于顶点顺序
...
bool InsideConvexPolygon(int vcount,FPOINT polygon[],FPOINT q) // 可用于三角形！     
{    
    FPOINT p;    
    LINESEG l;    
    int i;    
...
//判断点在多边形内，多边形可以不是简单多边形，可以不是凸多边形     
...
/*  
寻找凸包的Graham扫描法。凸包问题,就是给出一些点的坐标,然后求能围住所有这些点的最小周长。
PointSet为输入的点集----PointSet是输入输出参数，输出的点集是按照逆时针方向排列的。  
ch为输出的凸包上的点集----<del:按照逆时针方向排列> ，输出参数中的点可能有重复的。  
n为PointSet中的点的数目  
len为输出的凸包上的点的个数----len可能小于n
当PointSet调用前时已经是一个逆时针排列的简单多边形时，则输出的逆时针排列的简单多边形有可能顶点顺序改变了，有可能没变
*/
