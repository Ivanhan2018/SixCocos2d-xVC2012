...
void CCEGLViewProtocol::mouseEvent(float xs, float ys, unsigned int uMessage)
{
	if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32)
	{
		return;
	...
	pTouch->setTouchInfo(0, (xs - m_obViewPortRect.origin.x) / m_fScaleX, 
		(ys - m_obViewPortRect.origin.y) / m_fScaleY);
	set.addObject(pTouch);
	m_pDelegate->mouseEvent(&set, NULL, uMessage);
	// fixed memleak here!
	// Cool.Cat
