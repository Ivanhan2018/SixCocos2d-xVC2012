...
SIX_SocketClient::SIX_SocketClient(int protocol)
{
	...
	pConnectT = pReConnectT = pHeartBeatT = pRecvT = pSendT = 0;
	m_sendList = 0;
	m_iStatus = 0;
...
int SIX_SocketClient::init(char *ip,int port,char *name,char *pass,bool bReconnected,int iReconnectDelay)
{
	m_addr.clear();
	m_addr.append(ip);
	...
	// 默认设置断线重连
	this->setReconnect(bReconnected);
	// 默认设置断线重连间隔(ms)
	this->setReconnectDelay(iReconnectDelay);
...
void SIX_SocketClient::StartThread()
{
	...
	if (!this->checkStatus(THREAD_RECONNECT_RUNNING))
	{
		pReConnectT = new SIX_Thread();
		if (BeginThread(pReConnectT,(tc_thread_func)ReConnectThread/*,PTHREAD_CREATE_DETACHED*/)==0)
			this->setStatus(THREAD_RECONNECT_RUNNING);
	}
...
void SIX_SocketClient::StopThread()
{
	...
	// 如果连接线程存在，则等待连接线程自然退出
	while (this->checkStatus(THREAD_CONNECT_RUNNING)){}
	...
	{
		this->setStatus(-THREAD_RECONNECT_RUNNING);
		EndThread(pReConnectT);
		CC_SAFE_DELETE(pReConnectT);
	}
	if (this->checkStatus(THREAD_HEARTBEAT_RUNNING))
	{
		this->setStatus(-THREAD_HEARTBEAT_RUNNING);
...
{
	if (pThread && pThread->GetThreadID())
	{
		// 只有分离状态才等待线程自然结束
...
int SIX_SocketClient::ConnectThread(void *pointer)
{
	...
	// remove thread flag and end self variable
	// Cool.Cat
	pThis->setStatus(-THREAD_CONNECT_RUNNING);
	CC_SAFE_DELETE(pThis->pConnectT);

	return 1;
}

int SIX_SocketClient::ReConnectThread(void *pointer)
{
	SIX_SocketClient *pThis = static_cast<SIX_SocketClient*>(pointer);
	if (!pThis)
	{
		SIXLog("ReConnectThread.pThis.Nil");
		return -1;
	}

	SIXLog("ReConnectThread[0x%08X].Begin...",pThis->pReConnectT->GetThreadID());

	while (pThis->checkStatus(THREAD_RECONNECT_RUNNING))
	{
		// 设置了断线重连
		if (pThis->getReconnect())
		{
			// 未连接状态
			if (!pThis->getConnected() && !pThis->getConnecting())
			{
				// 尝试连接
	...
	SIXLog("ReConnectThread[0x%08X].Exit!",pThis->pReConnectT->GetThreadID());
