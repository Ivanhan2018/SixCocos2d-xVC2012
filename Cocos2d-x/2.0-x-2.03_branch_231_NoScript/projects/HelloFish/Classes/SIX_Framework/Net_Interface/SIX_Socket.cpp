...
{
	SIXLog("[%s|%d],waitTime=%d,Asyn=%d",addr,port,waitTime,Asyn);
    ...
    //remote_addr.sin_port = htons(port);
	remote_addr.sin_family = AF_INET;
	remote_addr.sin_addr.s_addr = *(unsigned long *)hp->h_addr_list[0];//inet_addr(addr);
	remote_addr.sin_port = htons(port);
#endif
	if (!Asyn)
	{
		SetAsynMode(false);
		/*
            手机连接局域网内的本机服务器会出现[Connect](177) ==> MT:Connect.Asyn[0].Error[(113)No route to host]
		*/
