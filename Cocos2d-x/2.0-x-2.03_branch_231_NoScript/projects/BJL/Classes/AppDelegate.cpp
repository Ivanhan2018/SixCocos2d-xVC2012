...
#include "gui/LoadingLayer.h"
#include "gui/ChongCoinsLayer.h"
...
{
    //初始化ShareSDK
    //C2DXShareSDK::open(CCString::create("5b021ac0b64a"), false);
	...
	handler->execute("create table if not exists AccountDB(id integer primary key, account TEXT, password TEXT)");
	//bool ret = handler->isTableExisted("FriendInfo");
	//if(!ret)
	//{
	//		handler->execute(
	//	"create table FriendInfo(ID integer primary key autoincrement, dwUserID integer, wFaceID integer, cbGender integer, cbMemberOrder integer, lScore integer, dwWinCount integer, dwLostCount integer, dwDrawCount integer, dwFleeCount integer, dwExperience integer, nickname TEXT)");
	//}

	//ret = handler->isTableExisted("UserInfo");
	//if(!ret)
	//{
	//		handler->execute(
	//	"create table UserInfo(ID integer primary key autoincrement, dwUserID integer, wFaceID integer, cbGender integer, cbMemberOrder integer, lScore integer, dwWinCount integer, dwLostCount integer, dwDrawCount integer, dwFleeCount integer, dwExperience integer, nickname TEXT)");
	//}

	// create a scene. it's an autorelease object
	CCScene *pScene = LoadingLayer::scene();
	//ChongCoinsLayer* layer = ChongCoinsLayer::ChongCoinsLayerWith(ccp(427,240));
...
void AppDelegate::applicationDidEnterBackground()
{
	...
	//{
	//	CCDirector::sharedDirector()->replaceScene(SceneControl::sharedSceneControl()->getScene(1,false));
	//}
