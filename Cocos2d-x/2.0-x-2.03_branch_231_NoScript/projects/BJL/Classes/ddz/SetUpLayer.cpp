...
bool SetUpLayer::init()
{
	...
	if(!m_voiceIsOpen)
	{
		m_voice = CCSprite::createWithSpriteFrameName(texture_name::s_on);//声音开底图
	}else{
		m_voice = CCSprite::createWithSpriteFrameName(texture_name::s_off);//声音关底图
	}
	m_voice->setAnchorPoint(ccp(0,0.5f));
	m_voice->setPosition(ccp(m_bgPoint.x-180.0f,m_bgPoint.y+85.0f));
	m_voiceOnOff = CCSprite::createWithSpriteFrameName(texture_name::s_on_off);//声音开关按钮
