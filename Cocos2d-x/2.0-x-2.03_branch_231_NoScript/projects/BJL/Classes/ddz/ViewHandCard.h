...
// 	CCLabelAtlas * m_leftCardNumAtlas;//左边玩家扑克牌数量
// 	CCLabelAtlas * m_rightCardNumAtlas;//右边玩家扑克牌数量

	CCNode * m_handCardNode;		//自己手牌node					--CCSpriteBatchNode有写手机不支持太多
	CCSpriteBatchNode * m_leftCardNode;		//上家手牌node
	CCSpriteBatchNode * m_rightCardNode;		//下家手牌node
	CCNode * m_diPaiNode;			//地主牌（底牌）node
	CCSpriteBatchNode * m_hintLeftCardNode;	//上家盖着的扑克牌node
	CCSpriteBatchNode * m_hintRightCardNode; //下家盖着的扑克牌node
	int m_dt[20];					//扑克牌偏移量
	CCNode * m_outCardNodeL;		//上家出的牌
	CCNode * m_outCardNodeR;		//下家出的牌
	CCNode * m_outCardNodeM;		//自己出的牌
	ViewTask*				m_viewTask;
	//--最大出牌数据
