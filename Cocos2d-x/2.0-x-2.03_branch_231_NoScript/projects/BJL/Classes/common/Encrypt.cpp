...
//生成密文
void CMD5Encrypt::EncryptData(unsigned char * pszSrcData, char szMD5Result[33])
{
	//加密密文
	CMD5 MD5Encrypt;
	unsigned char szResult[16];
	MD5Encrypt.MD5Update((unsigned char *)pszSrcData,strlen((const char*)pszSrcData)*sizeof(char));
...
//生成密文
WORD CXOREncrypt::EncryptData(unsigned char * pszEncrypData, unsigned char* pszSrcData, WORD wSize)
{
	//效验参数
//	ASSERT(wSize>0);
...
//	ASSERT(pszEncrypData!=NULL);
	...
	szRandKey[0]=strlen((const char *)pszSrcData);
	...
	for (WORD i=0;i<wTimes;i++)
	{
		...
		#ifdef WIN32
		_snprintf((char*)(pszEncrypData+i*4),5,"%02x%02x",szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
		#else
         snprintf((char*)(pszEncrypData+i*4),5,"%02x%02x",szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
		 #endif

        //char buf[50];
        //sprintf(buf,"%02x%02x",szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
...
//解开密文
WORD CXOREncrypt::CrevasseData(unsigned char* pszEncrypData, unsigned char* pszSrcData, WORD wSize)
{
	//效验参数
//	ASSERT(wSize>0);
//	ASSERT(pszSrcData!=NULL);
//	ASSERT(pszEncrypData!=NULL);
	...
	//效验长度
	WORD wEncrypPassLen=strlen((const char*)pszEncrypData);
	...
	for (int i=0;i<wSoureLength;i++)
	{
		...
		szTempBuffer[2]=0;
		unsigned int uKey=strtol((const char*)szKeyBuffer,&pEnd,16);
