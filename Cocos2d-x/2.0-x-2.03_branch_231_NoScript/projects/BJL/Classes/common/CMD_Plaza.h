...
{
	BYTE								cbAccountsType;					//’ ∫≈¿‡–Õ by ’≈ª™ 2010-8-24
	DWORD								dwPlazaVersion;					//π„≥°∞Ê±æ
	unsigned char						szAccounts[NAME_LEN];			//µ«¬º’À∫≈
...
struct CMD_GP_LogonSuccess
{
	...
	DWORD								sdServerID;						//被锁定服务ID 20100925 by张华
	DWORD								sdKindID;						//被锁定游戏ID 20100925 by张华
	LONG								lGoldEggs;						//用户金蛋 20100925 by张华
	LONG								lWeiWang;						//用户威望 20100925 by张华
	LONG								lWeekMingci;					//用户本周排名 20100926 by张华
	LONG								lWeekWinCount;					//用户本周距上一名局数 20100926 by张华

	char								szZipCode[ZIP_CODE_LEN];        //邮政编码 20100925 by张华
	char								szDwellingPlace[CITY_LEN];      //城市信息 20100925 by张华
	char								szDescribeString[DESCRIBE_LEN];	//描述消息 20100925 by张华
};
...
struct SDownloadGamePrize_Returnn : public cocos2d::CCObject
{
	LONG lReturn;
	DWORD dwDownloadGameInfo;
	DWORD dwCurScore;
