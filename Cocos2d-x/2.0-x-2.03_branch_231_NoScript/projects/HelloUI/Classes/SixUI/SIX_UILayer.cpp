...
int getPriority(CCTouchDispatcher* pDispatcher,CCTouchDelegate *pDelegate)
{
    CCAssert(pDelegate != NULL, "");

    CCTouchHandler *handler = NULL;

    handler = pDispatcher->findHandler(pDelegate);
...
SIX_UILayer::SIX_UILayer(bool bTouchThroughForbidden,int nPriority,bool bRedrawBorder)
{
...
SIX_UILayer * SIX_UILayer::create(const ccColor4B& color, float width, float height,bool bTouchThroughForbidden,int nPriority,bool bRedrawBorder)
{
    SIX_UILayer * pLayer = new SIX_UILayer(bTouchThroughForbidden,nPriority,bRedrawBorder);
...
SIX_UILayer * SIX_UILayer::create(const ccColor4B& color,bool bTouchThroughForbidden,int nPriority,bool bRedrawBorder)
{
    SIX_UILayer * pLayer = new SIX_UILayer(bTouchThroughForbidden,nPriority,bRedrawBorder);
...
{
	CCTouchDispatcher* pDispatcher = CCDirector::sharedDirector()->getTouchDispatcher();
	return ::getPriority(pDispatcher,this);
}
...
bool SIX_UILayer::getTouchEventPause()
{
	...
	return true;//handler->getTouchEventPause();
}
...
void SIX_UILayer::ccMouseMoved(CCTouch *pTouch,CCEvent *pEvent)
{
	if (m_ForbiddenTouch)
	{
		CCTouchDispatcher* pDispatcher = CCDirector::sharedDirector()->getTouchDispatcher();
		...
		CCARRAY_FOREACH_T(this->getChildren(),pChild,CCNode*)
		{
			...
			/*if (pHandler)
				pHandler->getDelegate()->ccMouseMoved(pTouch, pEvent);*/
		}
		return;
	}
	//CCLayerColor::ccMouseMoved(pTouch,pEvent);
}