...
PlayerLayer::PlayerLayer(int iPosition, bool bMySelf/*=false*/) : m_PlayerPT( iPosition )
	, m_iGunType( 1 )
...
void PlayerLayer::onEnter()
{
	...
	m_BulletManage->sharedBullet()->_setZOrder( Z_BULLET );
	...
	schedule(schedule_selector(PlayerLayer::OnRowingAnimation), 1.0f);
}
...
void PlayerLayer::ccMousesMoved(CCSet *touches, CCEvent *pEvent)
{
	CCSetIterator iter = touches->begin();
	for(; iter != touches->end(); iter++)
	{
...
void PlayerLayer::SetGunType(bool bAdd)
{
	if ( bAdd )
	{
	...
	char cBuffer[64]="";
	ANS_SNPRINTF(cBuffer, sizeof(cBuffer), "gun_%d_00000.png", m_iGunType);
	CCSprite * pShip = (CCSprite *)getChildByTag( t_Ship );
	...
	{
		char cBuffer[64]="";
		ANS_SNPRINTF(cBuffer, sizeof(cBuffer), "gun_%d_0000%d.png", m_iGunType, i);
...
void PlayerLayer::EmitGun(CCPoint pt)
{
	// 设置炮旋转
	CCSprite * pShip = (CCSprite *)getChildByTag( t_Ship );
	CCSprite * pGun = (CCSprite *)pShip->getChildByTag( t_Gun );
...
float PlayerLayer::whirlGun(CCPoint & pt)
{
	// 设置炮旋转
	CCSprite * pShip = (CCSprite *)getChildByTag( t_Ship );
	CCSprite * pGun = (CCSprite *)pShip->getChildByTag( t_Gun );
...
void PlayerLayer::OnRowingAnimation(float fTime)
{
	unschedule( schedule_selector(PlayerLayer::OnRowingAnimation) );
	...
	CCActionInstant * pComplete = CCCallFunc::create(this, callfunc_selector(PlayerLayer::OnRowingCompleteAnimation));
	CCFiniteTimeAction * pShipAnimation = CCSequence::create(pMove, pComplete, NULL);
	pShip->runAction( pShipAnimation );
}

void PlayerLayer::OnRowingCompleteAnimation()
{
	// 玩家划船进入，未到达捕鱼现场，除了炮与船其他信息暂隐藏。
	CCSprite *pChild = NULL;
	CCObject* pObj = NULL;
...
CBulletManage * PlayerLayer::getBulletManage()
{
...
PlayerLayer *PlayerManage::create(CCNode* pNode, int iPT, bool bMySelf/*=false*/)
{
	if (NULL == s_AllPlayer)
	{
	...
	PlayerLayer * pPlayer = new PlayerLayer( iPT, bMySelf );
...
PlayerLayer* PlayerManage::GetMyPlayer()
{
	if (-1 == m_MySelfID  ||  NULL == s_AllPlayer)
		return NULL;

	return (PlayerLayer*)s_AllPlayer->objectForKey( m_MySelfID );
}