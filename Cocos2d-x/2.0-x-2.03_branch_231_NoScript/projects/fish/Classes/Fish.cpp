...
CFish* CFish::createWithSpriteFrame(cocos2d::CCSpriteFrame *pSpriteFrame)
{
	CFish *pobSprite = new CFish();
...
void CFashManage::Release()
{
	if (NULL != s_AllFish)
	{
		...
		s_AllFish->removeAllChildrenWithCleanup(true);//add by Ivan_han 20140906removeAllChildren();
	}
}
