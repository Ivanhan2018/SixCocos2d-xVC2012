...
void CLoading::setProcessBar(float time, float fPercentage)
{
...
void CLoading::LoadComplete(float fTime)
{
	unschedule( schedule_selector(CLoading::LoadComplete) );
	CHall::scene();
}

void CLoading::onEnter()
{
	CCLayer::onEnter();
	...
	schedule( schedule_selector(CLoading::LoadComplete), fTime );
}

void CLoading::onExit()
{
	CCSpriteFrameCache::sharedSpriteFrameCache()->removeUnusedSpriteFrames();
}

//void CLoading::ccTouchesEnded(CCSet* touches, CCEvent* event)//del
//{
//	static int aa=0;
//	aa+=rand()%101;
//	if (aa > 100)
