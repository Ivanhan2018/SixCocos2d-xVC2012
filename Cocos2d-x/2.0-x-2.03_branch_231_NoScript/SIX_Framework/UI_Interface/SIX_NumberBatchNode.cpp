...
{
	m_llNumberLast = m_llNumber = m_uCellNum = m_uBitCount = m_uShowCount =0;
	m_uPadding = 0.0f;
...
void SIX_NumberBatchNode::setBitCount(unsigned int bitcount)
{
	...
	for (unsigned int i=0;i<m_uBitCount;i++)
	{
		// 默认为纹理的第一个cell
		CCSprite *pSprNum = CCSprite::create(this->pNumFile->getCString(),CCRect(0.0f,0.0f,this->getCellWidth(),this->getCellHeight()));
		if (!pSprNum)
		{
	...
	case ALIGN_LEFT:
		{
			...
			// 无效部分显示空纹理
			for (unsigned int i=pos;i<this->getBitCount();i++)
			{
				CCSprite *pSprNum = dynamic_cast<CCSprite*>(this->getChildren()->objectAtIndex(i));
				if (!pSprNum)
				{
