...
class SIX_Socket{
public:
	SIX_Socket(int protocol=IPPROTO_TCP);
	~SIX_Socket();
public:
	virtual void OnRecv(char *pData,int pLen){};
public:
	void InitSocket();
	bool SetAsynMode(bool isAsyn);
	bool Connect(const char* addr,unsigned int port,int waitTime=5,bool Asyn=false/*true*/);
	int Recv(int flag=0);
	int Send(const char* buf,int len,int flags=0);
	int close(CLOSE_TYPE iCloseType=CLOSE_TYPE_CLIENT_CLOSE);
	int GetErrorNumber();
	...
	void setCloseType(CLOSE_TYPE iCloseType);
...
private:
	...
	bool m_bConnected;
	bool m_bConnecting;
	CLOSE_TYPE m_iCloseType;
};

//// 以下来自linux 2.4.20-18的内核代码中的/usr/include/asm/errno.h
//// 与VS10自带的VC/include/errno.h有部分出入
//// Cool.Cat
