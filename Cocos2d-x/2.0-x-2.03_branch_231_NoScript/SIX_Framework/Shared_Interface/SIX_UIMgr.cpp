...
static bool LoadControl(SIX_UIMgr *pThis,T *pControl,SIX_DSTPL<SIX_KV> *pDataElement)
{
...
#endif
	{
		...
		//SIXLog("LoadControl.typeName[%s]",typeName);
	}
	...
	if (!STRCMP(typeName,"class cocos2d::CCLabelTTF *") || !STRCMP(typeName,"cocos2d::CCLabelTTF*"))
	{
		SIX_Utility::GetInstance()->FixedTextTexParameters(((CCLabelTTF*)pControl)->getTexture());
	}
	else if (!STRCMP(typeName,"class cocos2d::CCSprite *") || !STRCMP(typeName,"cocos2d::CCSprite*"))
	{
		SIX_Utility::GetInstance()->FixedImageTexParameters(((CCSprite*)pControl)->getTexture());
	}
	else if (!STRCMP(typeName,"class cocos2d::CCSpriteBatchNode *") || !STRCMP(typeName,"cocos2d::CCSpriteBatchNode*"))
	{
		SIX_Utility::GetInstance()->FixedImageTexParameters(((CCSpriteBatchNode*)pControl)->getTexture());
	}
...
static T *CreateControl(SIX_UIMgr *pThis,const char *name,SIX_DSTPL<SIX_KV> *pDataElement)
{
	...
	if (!STRCMP(typeName,"class cocos2d::CCSpriteBatchNode") || 
		!STRCMP(typeName,"cocos2d::CCSpriteBatchNode"))
	{
		// 先加载图片
		...
		pControl = (T*)CCSpriteBatchNode::create(pKVFile->getValue().getCString());
	...
	)
	{
		...
		int rcpos = pThis->GetAttributeAndPos(pDataElementKV,"tiledrect",pKVRect);
		if (rcpos>=0)
		{
			gotRc = true;
			tiledRc = true;
		...
		{
			rcpos = pThis->GetAttributeAndPos(pDataElementKV,"rect",pKVRect);
			if (rcpos>=0)
				gotRc = true;
		}
		...
		if (gotRc)
		{
			...
			if (count==4)
			{
				x = (float)atof(mp->at(0).c_str());
				y = (float)atof(mp->at(1).c_str());
				width = (float)atof(mp->at(2).c_str());
				height = (float)atof(mp->at(3).c_str());
			}
#ifdef WIN32
			mp->swap(MP());
#endif
		...
		{
			// 从文件加载
			if (pKVFile->getKey().compareIgnoreCase("file")==0)
			{
				if (rc.equals(CCRectZero))
				{
					if (!STRCMP(typeName,"class cocos2d::CCSprite") || !STRCMP(typeName,"cocos2d::CCSprite"))
				...
				{
					if (!STRCMP(typeName,"class cocos2d::CCSprite") || !STRCMP(typeName,"cocos2d::CCSprite"))
						pControl = (T*)CCSprite::create(pKVFile->getValue().getCString(),rc);
					else
			...
			else
			{
				...
				{
					if (!STRCMP(typeName,"class cocos2d::CCSprite") || !STRCMP(typeName,"cocos2d::CCSprite"))
						pControl = (T*)CCSprite::createWithSpriteFrameName(pKVFile->getValue().getCString());
					else
				...
				{
					if (!STRCMP(typeName,"class cocos2d::CCSprite") || !STRCMP(typeName,"cocos2d::CCSprite"))
						pControl = (T*)CCSprite::createWithSpriteFrameName(pKVFile->getValue().getCString()/*,rc*/);
					else
			...
			if (tiledRc)
			{
				...
				((CCSprite*)pControl)->getTexture()->setTexParameters(&tp);
				((CCSprite*)pControl)->ignoreAnchorPointForPosition(true);
			}
		}
...
bool SIX_UIMgr::ParseAttribute(CCNode *pControl,SIX_KV *pKV)
{
	...
	else if (!pKV->getKey().compareIgnoreCase("SkewY"))
	{
		pControl->setSkewY(pKV->getValue().floatValue());
			 ...
			 !pKV->getKey().compareIgnoreCase("PosRatio")  ||
		...
		case 2:
			{
				...
				// 增加偏移百分比解析，用于解决Android多分辨率机型的适配
				// Cool.Cat
				else if (!pKV->getKey().compareIgnoreCase("PosRatio"))
				{
					CCSize size = CCEGLView::sharedOpenGLView()->getVisibleSize();
					x *= size.width;
					y *= size.height;
					//SIXLog("CCNode.PosRatio.X[%.2f/%.2f].Y[%.2f/%.2f]",x,size.width,y,size.height);
					pControl->setPosition(ccp(x,y));
				}
		...
		case 2:
			{
				...
				// 增加偏移百分比解析，用于解决Android多分辨率机型的适配
				// Cool.Cat
				if (!pKV->getKey().compareIgnoreCase("PosRatio"))
				{
					CCSize size = CCEGLView::sharedOpenGLView()->getVisibleSize();
					x *= size.width;
					y *= size.height;
					//SIXLog("CCSprite.PosRatio.X[%.2f/%.2f].Y[%.2f/%.2f]",x,size.width,y,size.height);
					pControl->setPosition(ccp(x,y));
				}
				else if (!pKV->getKey().compareIgnoreCase("Pos"))
					pControl->setPosition(ccp(x,y));
...
bool SIX_UIMgr::ParseAttribute(CCLabelTTF *pControl,SIX_KV *pKV)
{
	...
	if (!pKV->getKey().compareIgnoreCase("String"))
	{
		...
		SIX_Utility::GetInstance()->FixedTextTexParameters(pControl->getTexture());
	}
	// 字体
...
// SIX_CardSuit属性
bool SIX_UIMgr::ParseAttribute(SIX_CardSuit *pControl,SIX_KV *pKV)
{
	if (!pControl || !pKV)
		return false;

	// 全部弹起
	if (!pKV->getKey().compareIgnoreCase("CardPopAll"))
	{
		pControl->setCardPopAll(pKV->getValue().boolValue());
	}
	// 全部激活
	else if (!pKV->getKey().compareIgnoreCase("CardActivedAll"))
	{
		pControl->setCardActivedAll(pKV->getValue().boolValue());
	}
	// 全部牌面显示
	else if (!pKV->getKey().compareIgnoreCase("CardFrontShowAll"))
	{
		pControl->setCardFrontShowAll(pKV->getValue().boolValue());
	}
	// 全部牌面显示
	else if (!pKV->getKey().compareIgnoreCase("CardReverseShowAll"))
	{
		pControl->setCardReverseShowAll(pKV->getValue().boolValue());
	}
	else
	{
...
// SIX_CardItem属性
bool SIX_UIMgr::ParseAttribute(SIX_CardItem *pControl,SIX_KV *pKV)
{
	if (!pControl || !pKV)
		return false;

	// 弹起
	if (!pKV->getKey().compareIgnoreCase("CardPop"))
	{
		pControl->setCardPop(pKV->getValue().boolValue());
	}
	// 激活
	else if (!pKV->getKey().compareIgnoreCase("CardActived"))
	{
		pControl->setCardActived(pKV->getValue().boolValue());
	}
	// 显示牌面
	else if (!pKV->getKey().compareIgnoreCase("CardFrontShow"))
	{
		pControl->setCardFrontShow(pKV->getValue().boolValue());
	}
	// 显示牌底
	else if (!pKV->getKey().compareIgnoreCase("CardReverseShow"))
	{
		pControl->setCardReverseShow(pKV->getValue().boolValue());
	}
	// 点数
	else if (!pKV->getKey().compareIgnoreCase("CardPointShow"))
	{
		pControl->setCardPointShow(pKV->getValue().boolValue());
	}
	// 小花色
	else if (!pKV->getKey().compareIgnoreCase("CardSmallFaceShow"))
	{
		pControl->setCardSmallFaceShow(pKV->getValue().boolValue());
	}
	// 大花色
	else if (!pKV->getKey().compareIgnoreCase("CardBigFaceShow"))
	{
		pControl->setCardBigFaceShow(pKV->getValue().boolValue());
	}
	// 子控件偏移
	else if (!pKV->getKey().compareIgnoreCase("CardPointPos") ||
			 !pKV->getKey().compareIgnoreCase("CardSmallFacePos") ||
			 !pKV->getKey().compareIgnoreCase("CardBigFacePos"))
	{
		MP *mp=new MP();
		int count = SIX_Utility::GetInstance()->Split(",",(char*)pKV->getValue().getCString(),mp);
		switch (count)
		...
		case 2:
			{
				float x = atof(mp->at(0).c_str());
				float y = atof(mp->at(1).c_str());
				if (!pKV->getKey().compareIgnoreCase("CardPointPos"))
					pControl->setCardPointPosition(ccp(x,y));
				else if (!pKV->getKey().compareIgnoreCase("CardSmallFacePos"))
					pControl->setCardSmallFacePosition(ccp(x,y));
				else if (!pKV->getKey().compareIgnoreCase("CardBigFacePos"))
					pControl->setCardBigFacePosition(ccp(x,y));
			}
		}
#ifdef WIN32
		mp->swap(MP());
	...
	{
		if (!ParseAttribute((CCControlButton*)pControl,pKV))
			return false;
	}
...
SIX_DSTPL<SIX_KV> *SIX_UIMgr::ParseKV(const char *name,SIX_DSTPL<SIX_KV> *pDataElement)
{
	...
	if (count==0)
	{
		...
		if (countSpace==0)
		{
			CCString *pElement = CCString::createWithFormat("%s",pKV->getValue().getCString());
			MP *Arr = new MP();
...
CCSprite *SIX_UIMgr::CreateSprite(CCNode *pControl,SIX_DSTPL<SIX_KV> *pDataElement)
{
	...
	if (pos>=0)
	{
		...
		bool gotRc = false;
		SIX_KV *pKVRect = new SIX_KV();
		int posRect = GetAttributeAndPos(pDataElement,"tiledrect",pKVRect);
		if (posRect>=0)
		{
			gotRc = true;
			tiledRc = true;
		...
		{
			posRect = GetAttributeAndPos(pDataElement,"rect",pKVRect);
			if (posRect>=0)
				gotRc = true;
		}
...
CCScale9Sprite *SIX_UIMgr::CreateScale9Sprite(CCNode *pControl,SIX_DSTPL<SIX_KV> *pDataElement)
{
	...
	if (pos>=0)
	{
		...
		if (posRect>=0)
		{
			...
			CCRect rc = CCRect(x,y,width,height);
			pControl = CCScale9Sprite::create(pKV->getValue().getCString(),rc);
			// 去除File属性
			pDataElement->Del(posRect);
		}
		else
...
// 新增2种牌控件
SIX_CardSuit *SIX_UIMgr::CreateCardSuit(CCNode *pControl,SIX_DSTPL<SIX_KV> *pDataElement)
{
	if (pControl || !pDataElement)
		return 0;

	pControl = new SIX_CardSuit();
	pControl->autorelease();

	// 再加载属性
	if (LoadControl<SIX_CardSuit>(this,(SIX_CardSuit*)pControl,pDataElement))
		return (SIX_CardSuit*)pControl;
	return 0;
}

SIX_CardItem *SIX_UIMgr::CreateCardItem(CCNode *pControl,SIX_DSTPL<SIX_KV> *pDataElement)
{
	if (pControl || !pDataElement)
		return 0;
	...
	if (pTTFNormal && bgImageNormal)
		pControl = SIX_CardItem::create(pTTFNormal,bgImageNormal);
	else if (pTTFNormal)
		pControl = SIX_CardItem::create(pTTFNormal,CCScale9Sprite::create());
	else if (bgImageNormal)
		pControl = SIX_CardItem::create(bgImageNormal);
	else
		pControl = SIX_CardItem::create("","",18);
	if (!pControl)
	{
	...
	if (pTTFHover)
		((SIX_CardItem*)pControl)->setTitleLabelForState(pTTFHover,CCControlStateHighlighted);
	// 掠过九宫格背景
	CCScale9Sprite *bgImageHover = CreateControl<CCScale9Sprite>(this,"hovScale9Sprite",pDataElement);
	if (bgImageHover)
		((SIX_CardItem*)pControl)->setBackgroundSpriteForState(bgImageHover,CCControlStateHighlighted);

	// 点击TTF
	CCLabelTTF *pTTFSelected = CreateControl<CCLabelTTF>(this,"selLabelTTF",pDataElement);
	if (pTTFSelected)
		((SIX_CardItem*)pControl)->setTitleLabelForState(pTTFSelected,CCControlStateSelected);
	// 点击九宫格背景
	CCScale9Sprite *bgImageSelected = CreateControl<CCScale9Sprite>(this,"selScale9Sprite",pDataElement);
	if (bgImageSelected)
		((SIX_CardItem*)pControl)->setBackgroundSpriteForState(bgImageSelected,CCControlStateSelected);

	// 禁用
	CCLabelTTF *pTTFDisabled = CreateControl<CCLabelTTF>(this,"disLabelTTF",pDataElement);
	if (pTTFDisabled)
		((SIX_CardItem*)pControl)->setTitleLabelForState(pTTFDisabled,CCControlStateDisabled);
	// 禁用九宫格背景
	CCScale9Sprite *bgImageDisabled = CreateControl<CCScale9Sprite>(this,"disScale9Sprite",pDataElement);
	if (bgImageDisabled)
		((SIX_CardItem*)pControl)->setBackgroundSpriteForState(bgImageDisabled,CCControlStateDisabled);

	// 最后解析SIX_CardItem自有属性
	// 批处理
	//// 先加载图片
	//SIX_KV *pBatchKV = new SIX_KV();
	//int pos = GetAttributeAndPos(pDataElement,"batchFile",pBatchKV);
	//if (pos>=0)
	//{
	//	
	//	((SIX_CardItem*)pControl)->m_pBatchNode = CCSpriteBatchNode::batchNodeWithFile(pBatchKV->getValue().getCString());
	//	// 去除batchFile属性
	//	pDataElement->Del(pos);
	//	if (((SIX_CardItem*)pControl)->m_pBatchNode)
	//	{
	//		// 再加载属性
	//		LoadControl<CCSpriteBatchNode>(this,(CCSpriteBatchNode*)(((SIX_CardItem*)pControl)->m_pBatchNode),pDataElement);
	//	}
	//}
	//CC_SAFE_DELETE(pBatchKV);

	((SIX_CardItem*)pControl)->m_pBatchNode = CreateControl<CCSpriteBatchNode>(this,"batchFile",pDataElement);
	// 正面背景
	((SIX_CardItem*)pControl)->m_pBackgroundFront = CreateControl<CCSprite>(this,"frontSprite",pDataElement);
	// 反面背景
	((SIX_CardItem*)pControl)->m_pBackgroundReverse = CreateControl<CCSprite>(this,"reverseSprite",pDataElement);
	// 点数
	((SIX_CardItem*)pControl)->m_pPoint = CreateControl<CCSprite>(this,"pointSprite",pDataElement);
	// 小花色
	((SIX_CardItem*)pControl)->m_pFaceS = CreateControl<CCSprite>(this,"faceSmallSprite",pDataElement);
	// 大花色
	((SIX_CardItem*)pControl)->m_pFaceB = CreateControl<CCSprite>(this,"faceBigSprite",pDataElement);

	((SIX_CardItem*)pControl)->DoCardCombined();

	// 再加载属性
	if (LoadControl<SIX_CardItem>(this,(SIX_CardItem*)pControl,pDataElement))
		return (SIX_CardItem*)pControl;
	return 0;
}
...
CCNode *SIX_UIMgr::ParseControl(SIX_XmlDataSet *pDataSet,CCNode *pRoot)
{
	...
	if (pDataSet->pDataElement)
	{
		...
		// 增加新控件CardSuit/CardItem
		else if (!pDataSet->getValue().compareIgnoreCase("CardSuit"))
		{
			pControl = CreateCardSuit(pControl,pDataSet->pDataElement);
			if (pControl)
				ControlIsValid = true;
		}
		else if (!pDataSet->getValue().compareIgnoreCase("CardItem"))
		{
			// 再解析SIX_CardItem属性
			pControl = CreateCardItem(pControl,pDataSet->pDataElement);
			if (pControl)
				ControlIsValid = true;
		}
