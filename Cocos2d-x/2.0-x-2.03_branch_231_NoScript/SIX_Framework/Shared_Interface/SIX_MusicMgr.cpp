...
{
	if (pDataSet->getValue().compareIgnoreCase(nodeName))
		return;
	...
	{
		if (pChild->getKey().compareIgnoreCase("Element") && pChild->getValue().compareIgnoreCase("Music"))
		{
		...
		if (pChild->getParent().compareIgnoreCase("BackgroundMusic")==0)
			pSound->setType(0);
		else if (pChild->getParent().compareIgnoreCase("EffectMusic")==0)
			pSound->setType(1);
		else
		{
		...
		{
			if (!pKV->getKey().compareIgnoreCase("id"))
			{
				col_count++;
				pSound->setId(pKV->getValue().intValue());
			}
			else if (!pKV->getKey().compareIgnoreCase("path"))
			{
				col_count++;
...
void SIX_MusicMgr::playBackgroundMusic(unsigned int nSoundId, bool bLoop)
{
	...
	if (pSound && pSound->getIsPreload())
	{
		...
		m_AudioEngine->setBackgroundMusicVolume(this->m_BackgroundVolume);
	}
}
...
{
	this->m_BackgroundVolume = volume;
...
unsigned int SIX_MusicMgr::playEffect(unsigned int nSoundId, bool bLoop)
{
	...
	{
		CCString *absPath = CCString::createWithFormat("%s/%s",m_SoundBasePath->getCString(),pSound->getPath().getCString());
		unsigned int retv =  m_AudioEngine->playEffect(absPath->getCString(),bLoop);
		m_AudioEngine->setEffectsVolume(this->m_EffectVolume);
		return retv;
	}
	return 0;
}
