...
public:
	...
	void setVisibleAllChildren(CCNode *pRoot,bool bVisible=true);
	...
	// 2D/3D纹理抗锯齿(文字)
	void FixedTextTexParameters(CCTexture2D *);
	// 2D/3D纹理抗锯齿(图片)
	void FixedImageTexParameters(CCTexture2D *);

	// 2D/3D纹理抗锯齿
	void FixedTexParameters(CCTexture2D *,bool isText=false);
	...
	static void CreateSixButtonWithFileCell(SIX_ControlButton ** pButton, const char *file,int cell,bool Hor=true);
