...
public:
	...
	// Cool.Cat@2013-05-02
		...
		sprintf(content, "file[%s].pXMLPath[%s].pXMLData[0x%08X]=%ld",file,pXMLPath,pXMLData,len);
		SIXLog("content=%s",content);
		if (pXMLData && len)
		{
			if (LoadXMLFromMemory(pXMLData,len)!=0)
			{
				sprintf(content,"file[%s].pXMLPath[%s].LoadXMLFromMemory.Error.len[%ld]",file,pXMLPath,len);
				SIXLog("content=%s",content);
			}
		}
#else
