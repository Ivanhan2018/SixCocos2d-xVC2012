...
int SIX_XMLParser::LoadXMLFromFile(const char *xmlPath)
{
	...
	pXMLPath = pFileUtils->fullPathFromRelativePath(xmlPath);

	// XML解析失败，格式异常等
	if (!xmlDoc->LoadFile(pXMLPath))
	{
		SIXLog("pXMLPath = %s",pXMLPath);
		SIXLog("LoadXMLFromFile.Parse.Error[%d]=%s",xmlDoc->ErrorId(),xmlDoc->ErrorDesc());
		retv=-2;
		goto doExit;
	}
