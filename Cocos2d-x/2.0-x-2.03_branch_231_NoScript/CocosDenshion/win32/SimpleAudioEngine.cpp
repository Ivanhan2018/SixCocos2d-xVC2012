...
#ifdef USING_DSOUND
typedef map<unsigned int,DXPlayer *> EffectList;
typedef pair<unsigned int,DXPlayer *> Effect;
#else
...
#ifdef USING_DSOUND
static DXPlayer &sharedMusic()
{
	static DXPlayer s_Music;
	return s_Music;
}
#else
...
#ifdef USING_DSOUND
        sharedList().insert(Effect(nRet, new DXPlayer()));
        DXPlayer * pPlayer = sharedList()[nRet];
#else
...
#ifdef USING_DSOUND
	EffectList::iterator p = sharedList().begin();
    for (;p != sharedList().end();p++)
    {
		DXPlayer *pPlayer = p->second;
		if (pPlayer)
