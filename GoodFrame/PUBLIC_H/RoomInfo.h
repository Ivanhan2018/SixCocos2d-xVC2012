...
struct CMD_WriteScoreBase
{
	...
	long long	lExp;			// 经验值
	int			iHighPlayerOperationType;	// 高级玩家操作类型(0:高级玩家未操作,1:通杀,2:杀大陪小)
};
...
struct CMD_ChatContent
{
	...
	int				iChatType;							// 聊天类型(0=普通广播(桌子/房间聊天,发给桌子/房间所有用户.房间聊天,发给房间所有用户); 1...(如充值消息广播、GM对警告等等))
