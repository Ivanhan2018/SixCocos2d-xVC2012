...
struct SEAT_INFO
{
	...
	//int			 iSeatNo;		// 座位号
	//bool		 bFreeSeat;		// 空座位
	unsigned int nOperateTime;	// 操作时间(【补鱼中用到过长时间未开火则踢出用户】【超时未准备则踢出用户】)
	unsigned int nSitDownTime;	// 坐下时间
...
// 换桌成功
struct CMD_SC_ChangeDeskSuc
{
	CMD_SC_ChangeDeskSuc()
	{
		memset(this, 0, sizeof(*this));
		iTableNo = INVAL_TABLE_NO;
		iSeatNo = INVAL_SEAT_NO;
