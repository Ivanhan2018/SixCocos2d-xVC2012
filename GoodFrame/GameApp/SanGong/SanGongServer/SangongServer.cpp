...
void show_prompt_msg()
{
	...
	sprintf(szPrompt[5],  "T7 打印房间用户列表信息 \n");
	sprintf(szPrompt[6],  "T8 打印所有桌子用户列表信息 \n");
	sprintf(szPrompt[7], "  \n");
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//读取xml文件
	int iSec=SangongConfig::Instance().GetCallBankerTime();
	...
	CTrunkServer::spawn(str.c_str());	// 孵化
	CTrunkServer* pService =  CTrunkServer::get_singleton_ptr();
	assert(pService);
	...
	while(cin.getline(szInput, INPUT_STRING_LEN))
	{
		...
		else if( 0 == strcmp("T3", szUpper))	// 列印消息
		{
			...
			// 列印所有的死循环、死锁
			pService->get_room_mgr()->display_all_guard();
		}
